...
struct AddProjectView: View {
    ...
    var body: some View {
        NavigationStack { Form { TextField("프로젝트명", text: $title); Picker("유형", selection: $type) { ForEach(ProjectType.allCases) { Text($0.title).tag($0) } }; Picker("상태", selection: $status) { ForEach(ProjectStatus.allCases) { Text($0.title).tag($0) } }; TextField("목표", text: $goal, axis: .vertical); if let errorMessage { Text(errorMessage).foregroundStyle(.red) } }.navigationTitle("새 프로젝트").toolbar { ToolbarItem(placement: .cancellationAction) { Button("취소") { dismiss() } }; ToolbarItem(placement: .confirmationAction) { Button { create() } label: { isCreating ? AnyView(ProgressView()) : AnyView(Text("생성")) }.disabled(isCreating || title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty) } } }
    }
