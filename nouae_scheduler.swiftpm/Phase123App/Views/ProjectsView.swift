...
struct ProjectsView: View {
    ...
    var body: some View {
        NavigationStack {
                    ...
                    if !items.isEmpty {
                        Section(status.title) {
                            ForEach(items) { project in
                                NavigationLink { ProjectDashboardView(project: project) } label: {
                                    ProjectCard(project: project, blocks: blocks.filter { $0.projectId == project.id }, logs: logs.filter { $0.projectId == project.id })
                                }
                            }
                        }
                    }
            ...
            .toolbar {
                ToolbarItem(placement: .topBarLeading) { Button { refresh() } label: { Image(systemName: "arrow.triangle.2.circlepath") } }
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Picker("유형", selection: $type) { ForEach(ProjectType.allCases) { Text($0.title).tag($0) } }
                Picker("상태", selection: $status) { ForEach(ProjectStatus.allCases) { Text($0.title).tag($0) } }
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) { Button { create() } label: { if isCreating { ProgressView() } else { Text("생성") } }.disabled(isCreating || title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty) }
            }
        }
    }
