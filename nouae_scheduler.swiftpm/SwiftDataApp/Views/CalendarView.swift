    ...
    private var filterMenu: some ToolbarContent {
        ToolbarItem(placement: .topBarTrailing) {
            Menu {
                ...
                Divider()
                ForEach(projects.filter { $0.calendarIdentifier != nil }) { project in Toggle(project.title, isOn: selectionBinding(project)) }
...
struct CalendarFlowBoard: View {
    ...
    private var monthDays: [Date] { guard let interval = Calendar.current.dateInterval(of: .month, for: date), let range = Calendar.current.range(of: .day, in: .month, for: date) else { return [] }; return range.compactMap { Calendar.current.date(byAdding: .day, value: $0 - 1, to: interval.start) } }
